 ...
 *
 * `unavailableColumns` sits outside that ladder: it names the columns an
 * extension has declared unavailable in the current context, and those are
 * dropped from the table entirely rather than merely defaulted to hidden.
 */
export function useViewOptionDefaults<T extends string | number | symbol>(
    ...
    const availabilityContext = {
        activeChannel: activeChannel ? { id: activeChannel.id, code: activeChannel.code } : undefined,
    };
    return {
        ...
        unavailableColumns: Object.entries(viewOptionDefaults?.columnAvailability ?? {})
            .filter(([, isAvailable]) => !isAvailable(availabilityContext))
